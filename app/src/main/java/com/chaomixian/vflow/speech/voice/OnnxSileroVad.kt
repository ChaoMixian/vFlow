...
class OnnxSileroVad(
    ...
    companion object {
        fun validateModel(modelFile: File) {
            ...
            env.createSession(modelFile.absolutePath, options).use { }
        }
    }
    ...
    init {
        require(modelFile.isFile) { "Silero VAD model file does not exist: ${modelFile.absolutePath}" }
        ...
        stateInputName = if (inputNameSet.contains("state")) "state" else null
        ...
        fun normalizeShape(shape: LongArray?, fallback: LongArray): LongArray {
            ...
            return LongArray(shape.size) { index ->
                val value = shape[index]
                if (value <= 0) 1L else value
            }
        }
        ...
        val srInfo = srInputName?.let(::getTensorInfo)
        ...
        val stateInfo = stateInputName?.let(::getTensorInfo)
        stateTensorShape = normalizeShape(stateInfo?.shape, longArrayOf(2, 1, 128))

        val hInfo = hInputName?.let(::getTensorInfo)
        val cInfo = cInputName?.let(::getTensorInfo)
    ...
    fun isSpeech(frame: ShortArray): Boolean {
        ...
        val audio = FloatArray(frameSize) { index -> frame[index] / 32768.0f }
        val modelInput = if (contextSize > 0) {
            val input = FloatArray(contextSize + frameSize)
            if (audioContext.isNotEmpty()) {
                System.arraycopy(audioContext, 0, input, 0, contextSize)
            }
            System.arraycopy(audio, 0, input, contextSize, frameSize)
            input
        } else {
            audio
        }

        val probability = predictProbability(modelInput)
        val isSpeechFrame = probability > threshold()

        if (contextSize > 0) {
            System.arraycopy(modelInput, modelInput.size - contextSize, audioContext, 0, contextSize)
        }
        ...
        try {
            ...
            session.run(inputs).use { out ->
                ...
                if (stateInputName != null && out.size() >= 2) {
                    extractFloatArray(out[1].value, 256)?.let { state = it }
                } else if (hInputName != null && cInputName != null && out.size() >= 3) {
                    extractFloatArray(out[1].value, 128)?.let { h = it }
                    extractFloatArray(out[2].value, 128)?.let { c = it }
                }

                return confidence
            }
        } finally {
            for (index in toClose.indices.reversed()) {
                runCatching { toClose[index].close() }
            }
        }
    }
